...
b=2 # 함수 밖에서 정의된 전역변수 b - 모든 곳에서 사용가능
# 함수 정의 후에 만들어 졌지만 함수 내부에서 사용 가능
show()
add()
print(a) # 전역변수는 모든 곳에서 사용가능
print(b) # 함수 내부/외부 모든곳에서 전역변수 사용가능
#print(c) # 전역변수가 정의되어 있더라도 사용전애 정의 되어 있어야함
c=3

## 전역변수가 정의된 위치는 함수 앞/ 다음 상관없음
...
# 함수 외부에서 전역변수를 이용해서 실행 할 때
# 실행전에 전역변수가 생성되어있어야 함