...
# print(crawling.find('a')) # 문자가 여러번 있어도 처음 찾은 문자의 위치를 반환

# index() : 문자열 내에서 특정 문자열의 시작위치를 반환
crawling = 'Data crawling is fun'
print(crawling.index('fun')) # 17 문자열 시작 위치 반환
print(crawling.index('f'))
# print(crawling.index('parsing')) # 문자열에 없으면 -1이 아닌 에러가 남
print(crawling.index('a')) # 문자가 여러번 있어도 처음 찾은 문자의 위치를 반환

# split() : 구분자를 기주으로 문자열을 나눔
# 나눈 결과를 리스트로 반환
# 구분자 : 기본 공백
# 구분자 지정 : "-", ":", ","
...
# replace() : 전체 문자열에서 특정 문자열을 찾아서 다른 문자열로 변경
# 전체 문자열.replace(찾는문자열, 변경할 문자열)
# 찾는 문자열이 존재하면 변경된 문자열 반환
# 찾는 문자열이 없으면 원래 문자열 그대로 반환
